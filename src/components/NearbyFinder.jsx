...
export default function NearbyFinder() {
  ...
  // request geolocation on mount and watch for changes
  useEffect(() => {
    ...
    const watchId = navigator.geolocation.watchPosition(
      (pos) => {
        ...
        // Only update if not manually overridden or if it's the first fix
        if (!sessionStorage.getItem('pawsafe_manual_location')) {
          setUserPos(coords);
        }
        setLoadingLocation(false);
        setError(null);
      ...
      (err) => {
        ...
        if (!userPos) {
          // Default to Mumbai if location is denied
          const defaultCoords = { lat: 19.0760, lng: 72.8777 };
          setUserPos(defaultCoords);
          setError('Location access denied. Using Mumbai as default.');
        }
        setLoadingLocation(false);
      },
      geoOptions
  ...
  const requestManualLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        const coords = { lat: pos.coords.latitude, lng: pos.coords.longitude };
        setUserPos(coords);
        sessionStorage.removeItem('pawsafe_manual_location');
        setLoadingLocation(false);
        setError(null);
      },
      (err) => {
  ...
  const onListClick = (place) => {
    // center map on place and show route
    if (place && place.lat && (place.lon || place.lng)) {
      setDestination({ lat: place.lat, lng: place.lon || place.lng });
  ...
  const handleManualLocationSelect = (loc) => {
    setUserPos({ lat: loc.lat, lng: loc.lng });
    sessionStorage.setItem('pawsafe_manual_location', JSON.stringify({ lat: loc.lat, lng: loc.lng }));
    setDestination(null);
  };
    ...
    <div className="nearby-root">
      ...
      <div className="main">
        <div className="map-panel">
          ...
          <div className="search-bar-container" style={{ position: 'absolute', top: '20px', left: '20px', right: '20px', zIndex: 1100, maxWidth: '400px' }}>
            <LocationAutocomplete onSelect={handleManualLocationSelect} placeholder="Search for your location..." />
          </div>

          <MapView center={userPos} places={places} onMarkerClick={onMarkerClick} highlightedPlaceId={highlighted} destination={destination} />
        </div>
